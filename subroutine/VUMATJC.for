c ======================================================================
c User Subroutine VUMAT for Johnson-Cook model and Jonhson-Cook damage model
c All rights of reproduction or distribution in any form are reserved.
c By Vu Hoai Lam --- mail: vuhoailam15081999@gmail.com
c ======================================================================
      subroutine vumat(
C Read only -
...
10    trInc = sum(strainInc(k, 1:3))
     ...
     1     alamda*trInc + 2.d0*mu*strainInc(k, k1)
      end do
      do k1 = ndir+1, ndir+nshr
          stressNew(k, k1) = stressOld(k, k1) 
     1     + 2.d0*mu*strainInc(k, k1)
      ...
      stateNew(k, 7) = 0.1d0               ! damage initialization
      tempNew(k) = Tr
      go to 30
C-------------------------- 2nd step and later -------------------------
20    epbar = stateOld(k, 2)              ! old plastic strain
      tempT = stateOld(k, 3)              ! old temperature
      sigyT = stateOld(k, 4)              ! old yield stress
      sOld(1:6) = stressOld(k, 1:6)       ! old stress
      trInc = sum(strainInc(k, 1:3))      ! trace strain increment

      do k1 = 1, ndir                     ! assign stress tensor
          sT(k1) = stressOld(k,k1) +alamda*trInc +
     1     2.d0*mu*strainInc(k, k1)
      end do 
      do k1 = ndir+1, ndir+nshr
          sT(k1) = stressOld(k, k1) + 
     1     2.d0*mu*strainInc(k, k1)
      end do 

c to get the vector for radial reduction
      sighyd = sum(sT(1:ndir))/3.d0       ! hydro stress 
      devS(1:ndir) = sT(1:ndir) - sighyd  ! compute deviation tensor
...
c rd is constant over the iterations
      ...
      sTem = matmul(cmat, rd) ! Temporary stress tensor

c initial value of the plastic strain increment and the bounds
      ep = 0.d0							! initial plastic strain incre.
      epmin = 0.d0
...
c updating stress
           sNew = sT - ep*sTem
c c calculate the new effective stress considering damage
c            if (stateNew(k, 7) .lt. 1.0) then
c                sNew(1:6) = sNew(1:6) ! * (1.d0 - stateNew(k, 7))  ! Effective stress
c            else
c                sNew(1:6) = 0.d0  ! Completely damaged
c            end if
c deviatoric stress
...
c evaluating new yield stress for the increment 'ep'
      call func_syield(A, B, epbarN, n, Tr, Tm, tempN, m, C,
     1 eprateN, rate0, sigyN)
      
      if (sigyN .lt. sigyT) then 
          sigyN = sigyT
      end if
...
c Damage evolution based on the equivalent stress and plastic strain 
          epfail =  D1 + D2*(2.71)**(sighyd*D3/sigeqv)
          ! stateNew(k, 7) = stateOld(k, 7) + ep/epfail
...
c ----------------------------------------------------------------------
      subroutine func_syield(A, B, epbar, n, Tr, Tm, T, m, 
     1 C, rate, rate0, sigbar)
          include 'vaba_param.inc'
          real*8 A, B, epbar, n, Tr, 
     1    Tm, T, theta, m, C, rate, rate0, sigbar
              ...
              sigbar = (A + B*epbar**n)*(1.d0 - 
     1         theta**m)*(1.d0 + C*log(rate/rate0)) 
      end subroutine
c ----------------------------------------------------------------------